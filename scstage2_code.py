...
We install and import all required packages for single-cell analysis.
"""
...
* statistical scoring and signatures (Decoupler)

* automated annotation (CellTypist)

* graph-based clustering (igraph)

* UMAP computation (fa2-modified)
...
#### The dataset contains a preprocessed gene-by-cell count matrix.
Inspecting obs and var reveals:

* cell metadata (UMI counts, QC metrics)

* gene metadata (names, IDs)

This confirms the data is compatible with Scanpy's AnnData format.
"""
...
High QC is essential to remove stressed, damaged, or low-information cells.

#### 3.1 Annotate QC-relevant gene categories
"""
...
"""| Gene set           | Biological Interpretation          | Reason for Filtering                                 |
| ------------------ | ---------------------------------- | ---------------------------------------------------- |
| Mitochondrial (MT) | Indicators of stressed/dying cells | High %MT suggests apoptosis                          |
| Ribosomal          | Reflect global transcription       | Excessive ribosomal content indicates imbalanced RNA |
| Hemoglobin         | Erythroid contamination            | Excess HB suggests red blood cell fragments          |

#### 3.2 Compute QC metrics
"""
...
"""## 4. Filtering
#### 4.1 Remove low-quality cells and genes
"""
...
"""Interpretation
* Removes cells with insufficient transcript complexity and rare genes that contribute noise.

#### 4.2 Remove cells with high mitochondrial / ribosomal / hemoglobin content
"""
...
#### Plot interpretation: Violin plots
"""
...
* Left: number of detected genes — low values indicate poor-quality cells.

* Middle: total UMI counts — helps detect outliers and potential doublets.

* Right: mitochondrial percentage — cells above ~20% likely stressed/dying.
"""
...
Doublets mimic hybrid cell types and distort clustering.
"""
...
"""### Plot interpretation: QC by doublet class"""
...
* Doublets often have inflated gene/UMI counts.

* Removing them ensures reliable clustering.
"""
...
These steps stabilize variance and emphasize biologically informative genes.
"""
...
"""#### Plot interpretation: HVG plot"""

sc.pl.highly_variable_genes(bm_adata)

"""Interpretation
* Highly variable genes represent key drivers of transcriptional heterogeneity and define clusters.
...
* PCs capturing technical variation (e.g., MT genes) should be interpreted with caution.

#### 7.2 Neighborhood Graph + UMAP
"""
...
* UMAP shows the major transcriptional programs.
* Coloring by QC metrics confirms no technical biases drive clustering.
...
Each Leiden cluster represents cells with distinct expression programs.
Changing resolution helps inspect biological granularity.
"""
...
"""## 9. Cell-Type Scoring (Decoupler + PanglaoDB)
Why;

Signature scoring validates clustering by linking each cluster to known immune cell identities.
"""
...
"""### Interpretation
Shows which clusters enrich for:
...
* plasma cell genes etc.

This helps validate automated labels.
"""
...
"""## 10. Automated Annotation (Top Signature per Cluster)"""
...
Clusters visually align with major immune cell types such as:
...
"""## 11. Marker Gene Visualization

These plots confirm whether identified cell types express canonical markers.
...
* Neutrophil clusters express S100A8/S100A9.

* Plasma cells express XBP1, MZB1, SDC1.

* NK cells express NKG7, GNLY, PRF1.

* B cells express MS4A1, CD79A.

This is strong evidence that annotations are biologically correct.
"""
...
"""## 12. Final Cell-Type Distribution"""
